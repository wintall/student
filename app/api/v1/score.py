...
@router.post("")
def create(body: ScoreCreate, db: Session = Depends(get_db), _: User = Depends(get_current_user)):
...
@router.put("/{score_id}")
def update(score_id: int, body: ScoreUpdate, db: Session = Depends(get_db), _: User = Depends(get_current_user)):
    s = score_service.update_score(score_id, body, db)
    return success(data={"id": s.id})


@router.delete("/{score_id}")
def delete(score_id: int, db: Session = Depends(get_db), _: User = Depends(get_current_user)):
