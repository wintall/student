...
@router.post("")
def create_role(body: RoleCreate, db: Session = Depends(get_db), _: User = Depends(get_current_user)):
...
@router.put("/{role_id}")
def update_role(role_id: int, body: RoleUpdate, db: Session = Depends(get_db), _: User = Depends(get_current_user)):
...
@router.post("/menus")
def create_menu(body: MenuCreate, db: Session = Depends(get_db), _: User = Depends(get_current_user)):
...
def update_menu(menu_id: int, body: MenuUpdate, db: Session = Depends(get_db), _: User = Depends(get_current_user)):
    menu = role_service.update_menu(menu_id, body, db)
    return success(data={"id": menu.id})


@router.delete("/menus/{menu_id}")
def delete_menu(menu_id: int, db: Session = Depends(get_db), _: User = Depends(get_current_user)):
...
def menu_tree(db: Session = Depends(get_db), _: User = Depends(get_current_user)):
    menus = role_service.get_all_menus(db)
    from app.core.permissions import _build_tree
    menu_list = [
        {"id": m.id, "parent_id": m.parent_id, "name": m.name, "code": m.code,
...
def assign_menus(body: dict, db: Session = Depends(get_db), _: User = Depends(get_current_user)):
