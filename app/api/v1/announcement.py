...
@router.post("")
def create(body: AnnouncementCreate, db: Session = Depends(get_db), user: User = Depends(get_current_user)):
...
@router.get("/{ann_id}")
def get(ann_id: int, db: Session = Depends(get_db), _: User = Depends(get_current_user)):
    ann = announcement_service.get_announcement(ann_id, db)
    return success(data=announcement_to_dict(ann))


@router.put("/{ann_id}")
def update(ann_id: int, body: AnnouncementUpdate, db: Session = Depends(get_db), user: User = Depends(get_current_user)):
    ann = announcement_service.update_announcement(ann_id, body, user.id, db)
    return success(data={"id": ann.id})


@router.delete("/{ann_id}")
def delete(ann_id: int, db: Session = Depends(get_db), user: User = Depends(get_current_user)):
