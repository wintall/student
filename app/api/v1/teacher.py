...
@router.post("")
def create(body: TeacherCreate, db: Session = Depends(get_db), _: User = Depends(require_permission("people:teacher:create"))):
...
def list_all(
    ...
    _: User = Depends(require_permission("people:teacher:list")),
):
...
def get(teacher_id: int, db: Session = Depends(get_db), _: User = Depends(require_permission("people:teacher:list"))):
    t = teacher_service.get_teacher(teacher_id, db)
    return success(data=teacher_to_dict(t))


@router.put("/{teacher_id}")
def update(teacher_id: int, body: TeacherUpdate, db: Session = Depends(get_db), _: User = Depends(require_permission("people:teacher:update"))):
    t = teacher_service.update_teacher(teacher_id, body, db)
    return success(data={"id": t.id})


@router.delete("/{teacher_id}")
def delete(teacher_id: int, db: Session = Depends(get_db), _: User = Depends(require_permission("people:teacher:delete"))):
